...
<nav class="navbar navbar-top navbar-expand-md navbar-dark d-md-flex d-none" id="navbar-main">
    <div class="container-fluid my-2">
        <!-- Brand -->
        <a class="h4 mb-0 text-white text-uppercase d-lg-inline-block" href="{{ route('home') }}">{{ __tr('Dashboard') }}</a>
        @if(session('loggedByVendor'))
        <a data-method="post" href="{{ route('vendor.user.write.logout_as') }}" class="h4 mb-0 text-white d-none d-lg-inline-block lw-ajax-link-action" ><i class="fa fa-arrow-left"></i> {{ __tr('You (__userFullName__) are logged to this user account , click here to go back to your account', [
            '__userFullName__' => session('loggedByVendor.name')
        ]) }}</a>
        @elseif(session('loggedBySuperAdmin'))
        <a data-method="post" href="{{ route('central.vendors.user.write.logout_as') }}" class="h4 mb-0 text-white d-none d-lg-inline-block lw-ajax-link-action" ><i class="fa fa-arrow-left"></i> {{ __tr('You (__userFullName__) are logged to this vendor admin account , click here to go back to super admin section', [
                ...
                <a class="nav-link pr-0" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <div class="media align-items-center">
                        <div class="media-body ml-2 d-lg-block">
                            <i class="fa fa-user"></i> {{ getUserAuthInfo('profile.full_name') }}
                        </div>
                    </div>
                </a>
